...
export function ApplicationCard({ application, onEdit, onDelete, onAddEvent, onReject }: Props) {
    ...
    return (
        <div
            className="relative overflow-hidden p-4 rounded-xl shadow-sm bg-gradient-to-br from-secondary/80 via-card to-card border border-border/60 transition-all duration-200 hover:shadow-lg"
            style={{
                borderLeftWidth: "4px",
                borderLeftColor: urgency.cssColor,
                borderBottomWidth: "2px",
                borderBottomColor: urgency.cssColor,
            }}
        >
            {/* En-tête */}
            <div className="flex items-start justify-between gap-2">
                <div className="flex items-start gap-3 flex-1 min-w-0">
                    <CompanyLogo company={application.company} className="h-9 w-9 shrink-0 mt-0.5 rounded-lg" />
            ...
            <Calendar className="h-3 w-3" />
                ...
                {application.contractType && (
                    <Badge variant="outline" className="text-xs">{application.contractType}</Badge>
                )}
                {application.source && (
            ...
            {followupLabel && (
                <div className="flex items-center gap-1.5 mt-2">
                    <Timer className={`h-3.5 w-3.5 ${application.isOverdue ? "text-red-500" : "text-muted-foreground"}`} />
                    <span className={`text-xs font-medium ${
                        application.isOverdue ? "text-red-500" :
                            (application.daysUntilFollowup != null && application.daysUntilFollowup <= 2) ? "text-orange-500" :
            ...
            {lastEvent && (
                <div className="mt-3 pt-3 border-t">
                    <p className="text-xs text-muted-foreground">
                        Dernier événement : {lastEvent.note || lastEvent.type}{" — "}{formatDate(lastEvent.date)}
                    </p>
                </div>
            )}
            ...
            {application.notes && (
                <div className={`mt-3 ${!lastEvent ? "pt-3 border-t" : ""}`}>
                    ...
                    >
                        <ChevronDown className={`h-3.5 w-3.5 transition-transform duration-200 ${showNotes ? "rotate-180" : ""}`} />
                        <span>{showNotes ? "Masquer les notes" : "Voir les notes"}</span>
                    </button>

                    <div className={`grid transition-all duration-200 ease-in-out ${showNotes ? "grid-rows-[1fr] opacity-100 mt-2" : "grid-rows-[0fr] opacity-0"}`}>
